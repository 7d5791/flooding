...
*///////////////////////////////////////////////*/
long  Mac1609_4::getReceivedPackets () {
    return statsReceivedPackets;
}

long Mac1609_4::getReceivedBroadcasts () {
    return statsReceivedBroadcasts;
}

long Mac1609_4::getSentPackets () {
     return statsSentPackets;
}

long Mac1609_4::getTXRXLostPackets () {
     return statsTXRXLostPackets;
}

long Mac1609_4::getSNIRLostPackets () {
     return statsSNIRLostPackets;
}

long Mac1609_4::getDroppedPackets () {
     return statsDroppedPackets;
}

long Mac1609_4::getNumTooLittleTime () {
     return statsNumTooLittleTime;
}

long Mac1609_4::getNumInternalContention () {
     return statsNumInternalContention;
}

long Mac1609_4::getNumBackoff () {

    return statsNumBackoff;
}

long Mac1609_4::getSlotsBackoff () {
     return statsSlotsBackoff;
}
...
simtime_t Mac1609_4::getMyIdleTime () {
      return hIdleTime;
}


long Mac1609_4:: getNumSuccessfullTx(){
      return StatsNumSuccessfullTx;
}


       // void Mac1609_4::setMyIdleTime (simtime_t t) {
          //          hIdleTime=t;
           //    }

        double Mac1609_4::gethRecvPower_dBm() {
             return hRecvPower_dBm;
        }
        double Mac1609_4::gethBitrate() {
             return hBitrate;
        }
        double Mac1609_4::gethSnr() {
             return hSnr;
         }
        ...
        long  Mac1609_4::gethSlotsBackoff(){
            hSlotsBackoff=myEDCA[activeChannel]->statsSlotsBackoff;
            return hSlotsBackoff;
        }

        long  Mac1609_4::gethNumInternalContention(){
            hNumInternalContention=myEDCA[activeChannel]->statsNumInternalContention;
            return hNumInternalContention;
         }



        void Mac1609_4::setMyDIF(simtime_t chdif)
        {
...
void Mac1609_4::handleLowerMsg(cMessage* msg) {
	...
	double hRecvPower_dBm=res->getRecvPower_dBm();
	double hBitrate= res->getBitrate();
	double hSnr= res->getSnr();
...
simtime_t Mac1609_4::EDCA::startContent(simtime_t idleSince,bool guardActive) {
	...
	for (std::map<t_access_category, EDCAQueue>::iterator iter = myQueues.begin(); iter != myQueues.end(); iter++) {
		if (iter->second.queue.size() != 0) {
			...
			DBG << "DIFS ============== " <<DIFS<< std::endl;
			DBG << "second.aifsn ============== " << iter->second.aifsn<< std::endl;
			DBG << "second.currentBackoff ============== " << iter->second.currentBackoff << std::endl;
